...
'
'   Author Martin van der Burgt, ambusy@protonmail.com
'   Written between 2008 and 2017.
'   I don't believe this project is closed yet.
Namespace Rexx
...
#End If
        ...
        Private ExecutablePath As String
...
#End If
        ...
        Friend Sub Logg(ByVal s As String)
            ...
            End If
        End Sub
        Sub New(ByRef store As RexxCompData)
            CurrRexxRun = store
            If fn <> "" Then My.Computer.FileSystem.DeleteFile(fn)
        ...
        Protected Overrides Sub Finalize()
            If Not CurrRexxRun.InInterpret Then
                ...
                Next
                Streams.Clear()
                'If Not CurrRexxRun.RexxFile Is Nothing Then
                '    CurrRexxRun.RexxFile.Dispose()
                '    CurrRexxRun.RexxFile.Close()
                'End If
                MyBase.Finalize()
            End If
        End Sub
        Public Function CompileRexxScript(ByVal Filename As String) As Integer
            ...
            If Not CurrRexxRun.InInterpret Then ' if not an INTERPRET
            ...
            If RcComp = 0 Then
                ...
                If CurrRexxRun.IntCode.Count() > 0 And Not CurrRexxRun.InInterpret Then
                ...
                For i = EntriesInIntcode + 1 To CurrRexxRun.IntCode.Count() ' insert call addresses 
                    ...
                    If asm.f = fct.sig Then ' replace labelname by index in currRexxRun.IntCode
                        Dim nrVars As Integer = CurrRexxRun.IdName.Count()
                        Dim xi As Integer = SourceNameIndexPosition("NOVALUE", tpSymbol.tpProcedure, DefVars) ' Find NOVALUE: label
                    ...
                    If asm.f = fct.cal Then ' replace routinename by index in CurrRexxRun.IntCode
                        ...
                        End If
                        If asm.a = 0 Then ' routine not found yet
                            Dim fn As String = ExecutablePath
                            fn = fn & DefVars.Id & ".rex"
                            If File.Exists(fn) Then ' is an external rexx routine
                                asm.a = StoreLiteral(fn)
                                asm.f = fct.cle
                            Else
                                fn = RexxFileName.Substring(0, RexxFileName.LastIndexOf("\"c) + 1)
                                fn = fn & DefVars.Id & ".rex"
                                If File.Exists(fn) Then ' is an external rexx routine
                                    asm.a = StoreLiteral(fn)
                                    asm.f = fct.cle
        ...
        Private Sub CompileOneStatement()
            ...
            Else
                ...
                ElseIf (cSymb = Symbols.leavesym Or cSymb = Symbols.itersym) Then  ' leave/iter
                    ...
                    End If
                    TestSymbolExpected(Symbols.semicolon, 118)
                ElseIf (cSymb = Symbols.signalsym) Then  ' signal on/off novalue  or  Signal label
                    GetNextSymbol()
...
#End If
        ...
        Private Sub FillRexxWords()
            ...
            i = i + 1 : RexxFunctions(i) = "00SSS      RXFUNCADD"
...
#End If
                Select Case cF
                    ...
                    Case fct.jbr ' builtin routines
                        ...
                        Select Case Mid(RexxFunctions(cA), 12)
                            ...
                            Case "REGEXP"
                                ...
                                End If
                            Case "RXFUNCADD"
                                Dim rMod As String = ps(1)
                                Dim rLib As String = ps(2)
                                Dim rUsr As String = ps(3)
                                If CurrRexxRun.RxFuncs.Contains(rUsr) Then
                                    CurrRexxRun.RxFuncs.Remove(rUsr)
                                End If
                                CurrRexxRun.RxFuncs.Add(rLib & "/" & rMod, rUsr)
                        End Select
                    ...
                    Case fct.cal
                        If cA < 0 Then ' unresolved called name
                            DefVars = DirectCast(CurrRexxRun.IdName.Item(-cA), DefVariable)
                            If CurrRexxRun.RxFuncs.Contains(DefVars.Id) Then
                                ' call a RxFunc
                                Dim sr As String = DirectCast(CurrRexxRun.RxFuncs.Item(DefVars.Id), String)
                                Dim cmd As String = "RXFUNCCALL, " & sr
                                For i = 1 To asm.l
                                    Dim asmr As AsmStatement = DirectCast(CurrRexxRun.IntCode.Item(IntPp + i), AsmStatement)
                                    cmd = cmd & ", P " & CStr(asmr.l)
                                Next
                                Dim sav As String = CurrRexxRun.CAddress
                                CurrRexxRun.CAddress = "EXTERN"
                                DoExc(cmd)
                                CurrRexxRun.CAddress = sav
                            Else
                                If DefVars.Kind <> tpSymbol.tpUnknown Then ' signal once as unknown procedure
                                    SigErrorF(123, (DefVars.Id))
                                    DefVars.Kind = tpSymbol.tpUnknown
                                End If
                            End If
                            IntPp += asm.l ' skip parameters of call
                        Else
                            SaveRegs()
                            IntPp = cA
                    ...
                    Case fct.upc 'upper/lower
                        ...
                        End If
                    Case Else
                        MsgBox("Internal error in REXX interpretor: " & CStr(cF) & " not implemented yet." & vbCrLf & "Current ReXXfile is: " & RexxFileName)
                End Select
                NrStepsExecuted += 1
...
#End If
        ...
        Sub DoExc(m As String)
            ...
            If (CurrRexxRun.CAddress = "") Then CurrRexxRun.CAddress = "EXTERN"
            Logg("""" & CurrRexxRun.CAddress.ToUpper(CultInf) & """")
            Dim savLenStack as integer = Stack.Count
            SaveRegs()
            Dim savRun As RexxCompData = CurrRexxRun
            Dim e As New RexxEvent
            RaiseEvent doCmd(CurrRexxRun.CAddress.ToUpper(CultInf), m, e, Me)
            Dim result As String = GetVar(CurrRexxRun.iRes, en, n) ' pass the RESULT to the caller
            CurrRexxRun = savRun
            savRun = Nothing
        ...
        Private Sub SaveRegs()
            ...
            Stack.Add(IntPp)
#If DEBUG Then
            Debug.WriteLine("save stack: pp=" & CStr(IntPp) + " cnt=" + CStr(Stack.Count))
#End If
        End Sub
        Private Function RestRegs() As Short
            IntPp = CInt(FromStack())
#If DEBUG Then
            Debug.WriteLine("from stack: pp=" & CStr(IntPp) + " cnt=" + CStr(Stack.Count + 1))
#End If
...
#End If
            ...
            If CurrRexxRun.InteractiveTracing = 1 Then
                ...
                Do While s.Trim <> ""
                            ...
                            If cF = fct.lin Then
                                If cL = nl Then
                                    If sUpper(0) = "G"c Then
                                        IntPp = lIntPP - 1 ' interpreter adds 1
                                        Exit Sub
                    ...
                    Else
                        ...
                        sTrace = RexxTrace
                        RexxTrace = False
                        'Dim crIntpt As New InterpretFromRexx(wfn, "", CurrRexxRun)
                        SaveRegs()
        ...
        Private Sub TracePLin(ByVal i As Integer) ' trace source after jmp, jcf, cal, lbl
            ...
            End If
        End Sub
        Private Sub argGen(ByRef arg As fct) ' gen code for Parse templates ARG, PULL, etc
                    ...
                    If currentToken.l = l1 Then
                        If MainParmIsRout Then
                            If l1 > ExtRoutParm.Count Then
                                m = ""
                            Else
                                m = DirectCast(ExtRoutParm.Item(l1), String)
                                If (UpCase) Then m = m.ToUpper(CultInf)
                            End If
                        End If
        ...
        Private Function ReadSourceFile(src As Collection, Filename As String) As Integer
            ...
            Try
                    ...
                    Else
                        ...
                        Else
                            If (ch = 10 Or ch = 13) Then
                                ...
                                Else
                                    ssd.SrcLength = CInt(iFile - ssd.SrcStart)
                                    SourceList.Add(ssd)
                ...
                For Each ssd In SourceList
                    ...
                    src.Add(s)
                Next
                RexxSourceFile.Close()
                RexxSourceFile.Dispose()
            Catch E As Exception
                rc = 16
            End Try
            Return rc
        ...
        Private Function ReadOneRexxLine(ByVal ssd As SourceLine, IsUnicode As Boolean, RexxSourceFile As FileStream) As String
            ...
            Return value
        End Function
        Friend Sub RexxCmd(ByVal Env As String, ByVal Cmd As String, ByVal e As RexxEvent, ByRef RexxEnv As Rexx)
            RaiseEvent doCmd(Env, Cmd, e, Me)
            CurrRexxRun.Rc = e.rc
        End Sub
    End Class
    ...
    Friend Class InterpretFromRexx
        Public WithEvents Rx As New Rexx(Nothing)
        Public Sub New(ByVal f As String, ByVal p As String, ByVal st As RexxCompData)
            'Dim SaveRexxFile As FileStream
            Rx.CurrRexxRun = st
            'SaveRexxFile = st.RexxFile
            If Rx.CompileRexxScript(f) = 0 Then
                'st.RexxFile = New FileStream(SaveRexxFile.Name, FileMode.Open, FileAccess.Read)
    ...
    Public Class RexxCompData
        'Friend RexxFile As FileStream
        Friend idt As String = CStr(Date.Now)
        Friend IntCode As New Collection ' executable code
        Friend RexxSrcList As New Collection ' Source
        ...
        Friend CallStack As New Collection ' execution routine Stack
        Friend SrcPos As Integer ' position in current sourceline while compile scan
        ...
        Friend IdName As New Collection ' variable names and charact in source.
        Friend ExVrNames As New Collection ' variable names and charact in execution.
        Public IdExpose As Collection ' exposed names of current procedure
        Public IdExposeStk As New Collection ' Stack of IdExpose's per procedurelevel
        ...
        Friend RxFuncs As New Collection
        ...
        Protected Overrides Sub Finalize()
            ...
            For Each exp As Collection In IdExposeStk
                exp.Clear()
            Next
            IdExposeStk.Clear() ' Stack of IdEexpose per procedure
            ...
            ExVrNames.Clear() ' variable names and charact in execution.
            MyBase.Finalize()
        End Sub
    End Class
